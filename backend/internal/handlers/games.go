...
func CreateGame(w http.ResponseWriter, r *http.Request) {
	...
	// for _, c := range cfg.InitialCards {
	// 	cardID := uuid.New()
	// 	colID, ok := columnIDs[c.ColumnTitle]
	// 	if !ok {
	// 		response.RespondWithError(w, http.StatusInternalServerError, "unknown column "+c.ColumnTitle)
	// 		return
	// 	}
	// 	if _, err := tx.Exec(
	// 		`INSERT INTO cards
...
func GetGame(w http.ResponseWriter, r *http.Request) {
	...
	err = database.DB.QueryRow(q, id).Scan(&g.ID, &g.CreatedAt, &g.Day, &g.Columns)
...
func GetBoard(w http.ResponseWriter, r *http.Request) {
	...
	// only select the real columns on `cards`
	rows, err := database.DB.Query(`
        SELECT
            id,
            game_id,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			response.RespondWithError(w, http.StatusInternalServerError, response.ErrDatabaseError)
			return
		}
